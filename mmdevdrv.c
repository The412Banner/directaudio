/*
 * PE side of the DirectAudio driver (winedirectaudio.drv).
 *
 * Wine 10's mmdevapi resolves the device GUID mapping on the PE via
 * GetProcAddress: get_device_guid / get_device_name_from_guid (see
 * dlls/mmdevapi/mmdevapi_private.h DriverFuncs). All streaming goes
 * through the unixlib (directaudio.c, `#pragma makedep unix`); this file
 * only provides the two exports Wine 10's load_driver() requires and the
 * DllMain that connects the PE to its unixlib.
 *
 * Device GUIDs are persisted per driver in the registry, mirroring
 * winecoreaudio.drv/mmdevdrv.c.
 *
